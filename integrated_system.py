...
class IntegratedVisualRAPTORSystem:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'num_documents': 500,
            'colbert_config': {
                'encoder_type': 'standard',  # 'standard' (BLIP) or 'modern' (SigLIP)
                'text_model': 'intfloat/multilingual-e5-large',
                'vision_model': 'Salesforce/blip-image-captioning-base',
                'embedding_dim': 768,
                'use_cross_attention': True  # ColModernVBERT用
            },
            'visual_config': {
    ...
    def initialize_system(self):
        ...
        if EnhancedVisualProcessor is not None:
            ...
            print("   ✅ Enhanced Visual Processor initialized")
        else:
            print("   ⚠️ Enhanced Visual Processor not available, using basic processor")
        
        # 文書生成器初期化（オプション）
        print("   Initializing document generator...")
        if DisasterDocumentGenerator is not None:
            self.document_generator = DisasterDocumentGenerator(
                output_dir=str(data_dir / 'disaster_documents')
            )
            print("   ✅ Disaster Document Generator initialized")
        else:
            print("   ⚠️ Disaster Document Generator not available")
    ...
    def setup_benchmark_data(self) -> Dict[str, Any]:
        ...
        # 災害文書生成（JinaVDRBenchmarkを使用）
        print(f"   Generating {self.config['num_documents']} disaster documents...")
        documents = self.benchmark.create_synthetic_documents(self.config['num_documents'])
        
        # 関連性判定生成
        print(f"   Generating relevance judgments...")
        judgments = self.benchmark.generate_relevance_judgments()
        
        # データ保存
        print(f"   Saving benchmark data...")
    ...
    def load_visual_documents_from_benchmark(self) -> Dict[str, Any]:
        """ベンチマークデータからビジュアル文書を読み込み"""
        print("📄 Loading visual documents from benchmark...")
        ...
        return {'num_documents': len(visual_docs), 'documents': visual_docs}
    
    def process_visual_documents(self) -> Dict[str, Any]:
        """ビジュアル文書を処理（既存のテキストコンテンツを使用）"""
        ...
        print(f"   Using synthetic text content from {len(self.benchmark.documents)} documents")
        
        # 既に生成されたテキストコンテンツを持つ文書を使用
        processed_docs = self.benchmark.documents
